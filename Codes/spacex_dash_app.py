...
def get_pie_chart(entered_site):
    if entered_site == 'ALL':
        ...
        names='class', 
        title='Success launch rate at '+entered_site)
        return fig
# TASK 4:
# Add a callback function for `site-dropdown` and `payload-slider` as inputs, `success-payload-scatter-chart` as output
@app.callback(Output(component_id='success-payload-scatter-chart', component_property='figure'),
              [Input(component_id='site-dropdown', component_property='value'),Input(component_id="payload-slider", component_property="value")])
def get_scatter(entered_site,entered_range):
    if entered_site=='ALL':
        ...
        filtered_df=spacex_df[(spacex_df['Payload Mass (kg)']>=entered_range[0]) & (spacex_df['Payload Mass (kg)']<=entered_range[1])&(spacex_df['Launch Site']==entered_site)]
        fig =px.scatter(filtered_df,x='Payload Mass (kg)',y='class', color='Booster Version Category',
